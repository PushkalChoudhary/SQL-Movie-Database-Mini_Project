...
--1)Find the top 5 best rated movies.
SELECT m.title, COALESCE(r.rating, 0)
FROM movies m
LEFT JOIN ratings r
...
--2)Which movies have been rated above 9 from users from India?
SELECT m.title, COALESCE(r.rating, 0), COALESCE(u.country, 'NA')
FROM movies m
LEFT JOIN ratings r 
ON m.movie_id=r.movie_id
LEFT JOIN users u
ON r.user_id=u.user_id
WHERE COALESCE(r.rating, 0) > 9 AND u.country = 'India';

--3)How many movies belong to each genre?
SELECT g.genre_id, g.genre_name, COUNT(mg.genre_id) AS Total_Count
FROM genres g
...
--4)Which actor has acted in highest number of movies.
SELECT a.name, COUNT(ma.actor_id) AS Total_Times_in_Movies
FROM movies m
...
GROUP BY a.name
ORDER BY Total_Times_in_Movies DESC
LIMIT 1;
...
--6)Assign a release rank to each movie according to its release year.
SELECT title, release_year,
	RANK() OVER(ORDER BY release_year ASC) AS Released_Rank
FROM movies;

--7)Classify movies as 'Classic', 'Modern' or 'New' based on its release year.
SELECT title, release_year,
	CASE
...
--8)Recommend movie of the day.
SELECT m.title, m.release_year, g.genre_name, a.name, r.rating
FROM movies m
